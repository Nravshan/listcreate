...
let modal = document.querySelector('.modal_oyna')
...
function reload(data, place) {
    ...
    for (let item of data) {
        ...
        edit.onclick = () => {
            newNameView.innerHTML = 'Имя: ' + item.name
            newAgeView.innerHTML = 'Возраст: ' + item.age
            changeID = item.id
            modalToggle()
        }
    }
...
function tableHeadCreate(place) {
    ...
    No.innerHTML = 'No'
    sName.innerHTML = 'Имя студента'
    birth.innerHTML = 'Год рождение'
    act.innerHTML = 'Настройки'
